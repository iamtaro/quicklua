    ...
    int       nCmdShow)
{
    ...
    return PrompaluaPlayer::createAndRun();
}

int PrompaluaPlayer::createAndRun(void)
{
    PrompaluaPlayer *host = PrompaluaPlayer::sharedInstance();
    int ret = host->run();
    PrompaluaPlayer::purgeSharedInstance();
    return ret;
}

PrompaluaPlayer *PrompaluaPlayer::s_sharedInstance = NULL;

PrompaluaPlayer *PrompaluaPlayer::sharedInstance(void)
{
    if (!s_sharedInstance)
    {
        s_sharedInstance = new PrompaluaPlayer();
    }
    return s_sharedInstance;
}

void PrompaluaPlayer::purgeSharedInstance(void)
{
    if (s_sharedInstance)
    {
...
PrompaluaPlayer::PrompaluaPlayer(void)
: m_app(NULL)
, m_hwnd(NULL)
, m_exit(TRUE)
...
int PrompaluaPlayer::run(void)
{
    ...
    do
    {
        ...
        SetWindowTextA(m_hwnd, "prompalua-player");
...
void PrompaluaPlayer::loadProjectConfig(void)
{
    vector<string> args;
...
void PrompaluaPlayer::createViewMenu(void)
{
...
void PrompaluaPlayer::updateMenu(void)
{
    HMENU menu = GetMenu(m_hwnd);
    HMENU viewMenu = GetSubMenu(menu, 1);
...
void PrompaluaPlayer::updateWindowTitle(void)
{
}

void PrompaluaPlayer::relaunch(void)
{
...
void PrompaluaPlayer::writeDebugLog(const char *log)
{
...
// menu callback
void PrompaluaPlayer::onFileOpenProject(void)
{
...
void PrompaluaPlayer::onFileCreateProjectShortcut(void)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        psl->SetDescription(L"PrompaluaPlayer");
...
void PrompaluaPlayer::onFileProjectConfig(void)
{
...
void PrompaluaPlayer::onFileRelaunch(void)
{
    relaunch();
}

void PrompaluaPlayer::onFileExit(void)
{
    DestroyWindow(m_hwnd);
}

void PrompaluaPlayer::onViewChangeFrameSize(int viewMenuID)
{
...
void PrompaluaPlayer::onViewChangeOrientation(int viewMenuID)
{
    bool isLandscape = m_project.isLandscapeFrame();
...
void PrompaluaPlayer::onViewChangeZoom(int scaleMode)
{
    float scale = 1.0f;
...
void PrompaluaPlayer::onHelpAbout(void)
{
    DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_ABOUTBOX), m_hwnd, AboutDialogCallback);
}

// windows callback
LRESULT PrompaluaPlayer::WindowProc(UINT message, WPARAM wParam, LPARAM lParam, BOOL* pProcessed)
{
    int wmId, wmEvent;
    PrompaluaPlayer *host = PrompaluaPlayer::sharedInstance();
...
INT_PTR CALLBACK PrompaluaPlayer::AboutDialogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
